...
def process_csv_to_csv(in_dir_or_file_path: str, out_dir_path: str) -> None:
    ...
    for filename in (pb := tqdm(all_files)):
        ...
        ).to_csv(preprocessed_data_filename, sep="\t")
...
pd.DataFrame(
    {
        "train": df_train[mlb.classes_].sum(axis=0),
        "valid": df_valid[mlb.classes_].sum(axis=0),
    }
...
class CustomDataset(Dataset):
    def __init__(
        ...
        num_augmentations=1,
        device=None,
        save_path="augmented_texts.json",
    ):
        self.tokenizer = tokenizer
        self.df = df
        self.title = list(df["text"])
    ...
    def augment_all_texts(self):
        ...
        augmented_texts = []
        for batch in tqdm(augmentation_dataloader, "Augmenting batches..."):
            batch = list(batch)

            # Process synthetic texts
            synthetic_results = self.generator(
                batch,
                max_new_tokens=self.max_len // 2,
            ...
            for result in synthetic_results:
                if isinstance(result, list):
                    synthetic_texts.extend(
                        [r["generated_text"] for r in result if "generated_text" in r]
                    )
                elif isinstance(result, dict) and "generated_text" in result:
                    synthetic_texts.append(result["generated_text"])

            # Process paraphrased texts
            paraphrase_results = self.paraphraser(
                [f"paraphrase: {text}" for text in batch],
            ...
            for result in paraphrase_results:
                if isinstance(result, list):
                    paraphrase_texts.extend(
                        [
                            r["generated_text"] if "generated_text" in r else r["text"]
                            for r in result
                        ]
                    )
                elif isinstance(result, dict):
                    paraphrase_texts.append(
            ...
            for original, synthetic, paraphrase in zip(
                batch, synthetic_texts, paraphrase_texts
            ):
...
valid_dataset = CustomDataset(
    ...
    num_augmentations=1,
    device=device,
    save_path=os.path.join(INTERIM_DIR, "augmented_texts_valid.json"),
)
