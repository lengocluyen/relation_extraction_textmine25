...
from utils import load_test_raw_data, TARGET_COL, INTERIM_DIR, MODELS_DIR, submission_path
...
model_dict_state_path = os.path.join(model_dir_path, "MLTC_model_state_camembert_large.bin")
...
def load_csv(dir_or_file_path: str, index_col=None, sep=',') -> pd.DataFrame:
    ...
    return pd.concat(
        [pd.read_csv(filename, index_col=index_col, header=0, sep=sep) for filename in all_files],
        axis=0,
        ignore_index=True
    )
...
def process_data(data: pd.DataFrame) -> pd.DataFrame:
    return pd.concat(
        [data, pd.DataFrame(mlb.transform(data[TARGET_COL]), columns=mlb.classes_, index=data.index)],
        axis=1
    )
...
def process_csv_to_csv(in_dir_or_file_path: str, out_dir_path: str) -> None:
    ...
    for filename in tqdm(all_files, desc="Processing CSV files"):
        ...
        data = load_csv(filename).assign(**{TARGET_COL: lambda df: df[TARGET_COL].apply(format_relations_str_to_list)})
...
val_data_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=VALID_BATCH_SIZE, shuffle=False)
...
class TransformerAttentionBertModel(nn.Module):
    ...
    def forward(
        ...
        transformer_output = transformer_output.permute(1, 0, 2)  # Shape: [batch_size, seq_len, hidden_size]
        pooled_output = torch.mean(transformer_output, dim=1)  # Shape: [batch_size, hidden_size]

        # Pass through classification head
        logits = self.classification_head(pooled_output)  # Shape: [batch_size, n_classes]
...
for epoch in range(1, EPOCHS + 1):
    ...
    train_loss, train_acc = train_model(model, train_data_loader, optimizer, scheduler)
    val_loss, val_acc, val_f1_macro = eval_model(model, val_data_loader)
...
texts_test, predictions_test, prediction_probs_test, _ = get_predictions(model, test_data_loader)
    ...
    [
        ...
        pd.Series(
            mlb.inverse_transform(
                pd.DataFrame(predictions_test.numpy(), columns=target_list)[mlb.classes_].values
            ),
            name=TARGET_COL,
            index=df_test.index
        )
...
print(ml_labeled_test_df)

# Adjust as per your submission requirements
text_idx_to_relations = {
    text_index: [l[0] for l in group_df.drop(["text_index", "text"], axis=1, errors='ignore')[group_df[TARGET_COL].str.len() > 0].apply(
        lambda row: [[row.iloc[0], r, row.iloc[1]] for r in row.iloc[-1]] if len(row.iloc[-1]) > 0 else [],
        axis=1
    ).values.tolist()]
...
}).set_index("id").loc[load_test_raw_data().index]
